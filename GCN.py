...
class GConv(nn.Module):
    ...
    def __init__(self, input_dim, output_dim=None, hidden_dim=512, pooling='avg', mlp_normalization='none'):
        ...
        self.gconv.apply(self._init_weights)
    ...
    def _build_mlp(dim_list, activation='relu',batch_norm='none',dropout=0,final_nonlinearity=True):
        layers = []
    ...
    def forward(self, obj_vecs, pred_vecs, edges):
        ...
        _, D = obj_vecs.shape
        T, _ = edges.shape
        assert T == pred_vecs.shape[0]
        assert D == pred_vecs.shape[1]
        ...
        new_subject_vecs = new_triple_vecs[:, :Hidden] # (T, Hidden)
        new_pred_vecs = new_triple_vecs[:, Hidden:(Hidden + Dout)] # (T, Dout)
        new_object_vecs = new_triple_vecs[:, (Hidden + Dout):] # (T, Hidden)

        # Aggregate the new subject and object vectors
        new_obj_vecs = self._aggregate(new_subject_vecs, new_object_vecs, subjects_idx, objects_idx, obj_vecs.shape[0]) # (O, Dout)
    ...
    def _aggregate(self, new_s_vecs, new_o_vecs, s_idx, o_idx, Obj_vecs_shape_0):
        ...
        if self.pooling == 'avg':
            ...
            obj_counts = torch.zeros(Obj_vecs_shape_0, dtype=dtype, device=device)
            obj_counts = obj_counts.scatter_add(0, s_idx, torch.ones_like(s_idx, dtype=dtype, device=device))
            obj_counts = obj_counts.scatter_add(0, o_idx, torch.ones_like(o_idx, dtype=dtype, device=device))
...
class GCN(nn.Module):
    ...
    """
    def __init__(self, input_dim, hidden_dim=512, num_layers=5, pooling='avg', mlp_normalization='none'):
...
class GAttn(nn.Module):
    """
    A single layer of scene graph attention.
    """
    def __init__(self, input_dim, output_dim=None, hidden_dim=512, pooling='avg', mlp_normalization='none'):
        ...
        self.gattn1 = self._build_attn_mlp([3 * input_dim, hidden_dim, 2 * hidden_dim + output_dim], batch_norm=mlp_normalization)
        self.gattn1.apply(self._init_weights)

        self.gattn2 = self._build_attn_mlp([hidden_dim, hidden_dim, output_dim], batch_norm=mlp_normalization)
        self.gattn2.apply(self._init_weights)
    ...
    def _build_attn_mlp(self, dim_list, num_head=4, batch_norm='none', dropout=0, final_nonlinearity=True):
        layers = []
        for i in range(len(dim_list) - 1):
            input_dim, output_dim = dim_list[i], dim_list[i + 1]
            layers.append(nn.MultiheadAttention(input_dim, num_head))
            final_layer = (i == len(dim_list) - 2)
            if not final_layer or final_nonlinearity:
                if batch_norm == 'batch':
    ...
    def forward(self, obj_vecs, pred_vecs, edges):
        ...
        # Concatenate the subject, predicate, and object vectors
        current_triple_vecs = torch.cat([current_subject_vecs, pred_vecs, current_object_vecs], dim=1) # (T, 3*Din)
        new_triple_vecs = self.gattn1(current_triple_vecs) # (T, 2*Hidden + Dout)
        ...
        # Apply a second MLP to the new object vectors
        new_obj_vecs = self.gattn2(new_obj_vecs) # (O, Dout)
...
class GAttnNet(nn.Module):
    """
    A sequence of scene graph attention layers.
    """
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, pooling='avg', mlp_normalization='none'):
        super(GAttnNet, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.num_layers = num_layers
        ...
        self.gattn_layers = nn.ModuleList()
        for i in range(num_layers):
            layer = GAttn(input_dim, hidden_dim, output_dim, pooling, mlp_normalization)
            self.gattn_layers.append(layer)
    ...
    def forward(self, obj_vecs, pred_vecs, edges):
        ...
        """
        for i in range(self.num_layers):
            obj_vecs, pred_vecs = self.gattn_layers[i](obj_vecs, pred_vecs, edges)
        return obj_vecs, pred_vecs
